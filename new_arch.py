...
def ensure_tensor(input_array, device):
...
class AWE(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_dim, pretrained_embeddings, device):
        ...
        self.embedding.weight.data.copy_(ensure_tensor(pretrained_embeddings, device))
...
class LSTM(nn.Module):
    ...
    def forward(self, text):
        ...
        embeddings = self.embedding(text)
        packed = pack_padded_sequence(embeddings, lengths.to(device), batch_first=True, enforce_sorted=False)
